...
def main():
    ...
    model.load_state_dict(
        torch.load("checkpoints/mtl_joint.pt", map_location=device)
    )
    model.to(device)
    model.eval()
    ...
    task_id = TASKS["mmhs150k"]["id"]

    with torch.no_grad():
        for batch in tqdm(loader, desc="Evaluating Joint on MMHS150K"):
    ...
    print("\nJoint Model Accuracy on MMHS150K:", round(acc, 4))
    print("Confusion Matrix:")
    print(cm)

    np.save("evaluation/joint_mmhs150k_confusion_matrix.npy", cm)
