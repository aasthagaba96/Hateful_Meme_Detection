...
def main():
    ...
    task_id = TASKS["mami"]["id"]

    all_preds, all_labels = [], []

    with torch.no_grad():
        for batch in tqdm(loader, desc="Evaluating Joint on MAMI"):
            images = batch["images"].to(device)
            texts = batch["texts"]
            labels = batch["labels"]["label"].to(device)
    ...
    print(f"\nJoint Model Accuracy on MAMI (label): {acc:.4f}")
